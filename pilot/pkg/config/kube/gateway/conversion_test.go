...
func TestInferencePoolExtraConfigs(t *testing.T) {
	...
	byHostname := map[string]map[string]gatewaykube.InferencePoolRouteRuleConfig{}
	for _, vs := range ctrl.List(gvk.VirtualService, "") {
		configs, _ := vs.Extra[constants.ConfigExtraPerRouteRuleInferencePoolConfigs].(map[string]gatewaykube.InferencePoolRouteRuleConfig)
		for _, h := range vs.Spec.(*istio.VirtualService).Hosts {
			if byHostname[h] == nil {
				byHostname[h] = map[string]gatewaykube.InferencePoolRouteRuleConfig{}
			}
			for routeName, backends := range configs {
				for backendHost, backendConfig := range backends {
					if byHostname[h][routeName] == nil {
						byHostname[h][routeName] = gatewaykube.InferencePoolRouteRuleConfig{}
					}
					byHostname[h][routeName][backendHost] = backendConfig
				}
			}
		}
	}
	...
	for _, tt := range cases {
		t.Run(tt.hostname, func(t *testing.T) {
			got, found := byHostname[tt.hostname]
			if !found {
