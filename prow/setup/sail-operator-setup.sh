...
function install_ztunnel() {
  ...
  # Apply multicluster configuration
...
function install_istio(){
  ...
  patch_config
  oc apply -f "$WORKDIR/$SAIL_IOP_FILE" || { echo "Failed to install istio"; oc get istio default -o yaml;}
...
function install_gateways() {
  ...
  oc -n "$NAMESPACE" wait --for=condition=Available deployment/istio-egressgateway --timeout=60s || { echo "Failed to start istio-egressgateway";  oc get istios; oc get pods -n "$NAMESPACE" -o wide; exit 1;}
...
function patch_config() {
  ...
  # Workaround until https://issues.redhat.com/browse/OSSM-10480 fixed
    ...
    otel_cred="$(oc -n "$NAMESPACE" get secret otel-credentials -o jsonpath='{.data.bearer-token}' | base64 -d)"
  ...
  if [[ "$AMBIENT" == "true" ]]; then
    ...
    # Apply multicluster configuration for Ambient mode
...
function cleanup_istio() {
  ...
  oc delete istiocni --all -n "$ISTIOCNI_NAMESPACE" --wait=true --timeout=$TIMEOUT_DURATION || {
    echo "Normal delete failed for $ISTIOCNI_NAMESPACE or timed out, applying force delete..."
    oc delete all --all -n "$ISTIOCNI_NAMESPACE" --force --grace-period=0 --wait=true
  }

  echo "Deleting ZTunnel resources from namespace $ZTUNNEL_NAMESPACE..."
  oc delete ztunnel --all -n "$ZTUNNEL_NAMESPACE" --wait=true --timeout=$TIMEOUT_DURATION || {
    echo "Normal delete failed for $ZTUNNEL_NAMESPACE or timed out, applying force delete..."
    oc delete all --all -n "$ZTUNNEL_NAMESPACE" --force --grace-period=0 --wait=true
  }

  echo "Deleting Istio resources from namespace $NAMESPACE..."
  oc delete istio --all -n "$NAMESPACE" --wait=true --timeout=$TIMEOUT_DURATION || {
    echo "Normal delete failed for $NAMESPACE or timed out, applying force delete..."
    oc delete all --all -n "$NAMESPACE" --force --grace-period=0 --wait=true
  }

  echo "Delete Istio, IstioCNI and Ztunnel namespaces"
  oc delete namespace "$ISTIOCNI_NAMESPACE" || true
  oc delete namespace "$ZTUNNEL_NAMESPACE" || true
  oc delete namespace "$NAMESPACE" || true

  echo "Cleanup completed successfully."
}

# ==================== Multicluster: Topology Helpers ====================

function load_topology() {
  local topology_file="${INTEGRATION_TEST_TOPOLOGY_FILE:-${PROW}/artifacts/topology-config.json}"
  ...
  mapfile -t ALL_CLUSTER_KUBECONFIGS < <(jq -r '.[] | (.meta.kubeconfig // "")' "$topology_file")
  ...
  if [ "${#ALL_CLUSTER_KUBECONFIGS[@]}" -ne "${#ALL_CLUSTER_NAMES[@]}" ]; then
    echo "Error: topology arrays misaligned (${#ALL_CLUSTER_KUBECONFIGS[@]} kubeconfigs vs ${#ALL_CLUSTER_NAMES[@]} clusters)"
    exit 1
  fi
...
function switch_cluster() {
  ...
  if [ -z "$kubeconfig" ]; then
    echo "Error: no kubeconfig for cluster '$cluster_name'"
    return 1
  fi
...
function wait_for_injection_webhook() {
  ...
  while [ $attempts -lt $max_attempts ]; do
    local ca_bundle
    ca_bundle=$(oc get mutatingwebhookconfiguration istio-sidecar-injector \
...
function install_eastwest_gateway() {
  ...
  if [ "$AMBIENT" == "true" ]; then
    ...
    oc -n "$NAMESPACE" wait --for=condition=Programmed gateway/istio-eastwestgateway --timeout=120s \
      || { echo "East-west Gateway not programmed"; oc -n "$NAMESPACE" get gateway,svc -o wide; exit 1; }
...
function expose_istiod() {
  echo "Exposing istiod via east-west gateway..."
  oc apply -n "$NAMESPACE" -f "${ROOT}/samples/multicluster/expose-istiod.yaml"
  echo "istiod exposed via east-west gateway."
}
...
function get_eastwest_gateway_address() {
  ...
  local addr=""
  while [ $attempts -lt $max_attempts ]; do
    addr=$(oc -n "$NAMESPACE" get svc istio-eastwestgateway \
      -o jsonpath='{.status.loadBalancer.ingress[0].ip}' 2>/dev/null || true)
    if [ -n "$addr" ]; then
      echo "$addr"
      return
    fi
    echo "Waiting for east-west gateway external IP... (attempt $((attempts+1))/$max_attempts)" >&2
    sleep 10
    attempts=$((attempts+1))
  done
...
function install_multicluster() {
  ...
  for idx in "${!ALL_CLUSTER_NAMES[@]}"; do
    if is_remote "$idx" && ! is_config "$idx"; then
      ...
      local primary_idx
      primary_idx=$(get_primary_index "$idx") || return 1
      switch_cluster "$primary_idx"
      local pilot_addr
      pilot_addr=$(get_eastwest_gateway_address) || return 1
      local cluster_name="${ALL_CLUSTER_NAMES[$idx]}"
      local network="${ALL_CLUSTER_NETWORKS[$idx]}"

      switch_cluster "$idx"
      download_execute_converter_for_role "remote"
      yq eval '.spec.profile = "remote"' -i "$WORKDIR/$SAIL_IOP_FILE"
      set_cluster_identity "$idx"
      ...
      yq eval ".spec.values.istiodRemote.injectionPath = \"/inject/cluster/$cluster_name/net/$network\"" -i "$WORKDIR/$SAIL_IOP_FILE"
  ...
  for idx in "${!ALL_CLUSTER_NAMES[@]}"; do
    if is_remote "$idx" && is_config "$idx"; then
      ...
      switch_cluster "$idx"
      oc patch istio default -n "$NAMESPACE" --type merge \
...
function cleanup_multicluster() {
  ...
  for idx in "${!ALL_CLUSTER_NAMES[@]}"; do
    ...
    if switch_cluster "$idx"; then
      cleanup_istio || {
        echo "Warning: cleanup failed for cluster ${ALL_CLUSTER_NAMES[$idx]}, continuing..."
        cleanup_failed=1
      }
    else
      echo "Warning: could not switch to cluster ${ALL_CLUSTER_NAMES[$idx]}, skipping cleanup"
      cleanup_failed=1
    fi
  done
  if [ ${cleanup_failed} -ne 0 ]; then
    echo "Warning: one or more cluster cleanups failed"
  fi
}
