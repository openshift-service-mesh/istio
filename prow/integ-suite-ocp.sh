...
if [[ "${TOPOLOGY}" != "SINGLE_CLUSTER" ]]; then
  ...
  if [[ "${CLUSTER_TOPOLOGY_CONFIG}" == "prow/config/topology/ocp/multicluster.json" ]] || [[ "${CLUSTER_TOPOLOGY_CONFIG}" == "prow/config/topology/ocp/ambient-multicluster.json" ]]; then
    # Count available clusters by number of kubeconfig files
    IFS=':' read -r -a AVAILABLE_KUBECONFIGS <<< "${KUBECONFIG}"
    NUM_AVAILABLE_CLUSTERS=${#AVAILABLE_KUBECONFIGS[@]}
...
if [[ "${AMBIENT}" == "true" || "${TEST_SUITE}" == *"ambient"* ]]; then
    ...
    gw_kubeconfigs=("")
    if [[ "${TOPOLOGY}" != "SINGLE_CLUSTER" ]] && [[ -n "${KUBECONFIG:-}" ]]; then
        IFS=':' read -r -a gw_kubeconfigs <<< "${KUBECONFIG}"
    fi
    for kconfig in "${gw_kubeconfigs[@]}"; do
        kconfig_args=()
        kconfig_display="current cluster"
        if [[ -n "${kconfig}" ]]; then
            kconfig_args=("--kubeconfig=${kconfig}")
            kconfig_display="${kconfig}"
        fi
        echo "Setting local gateway mode on ${kconfig_display}"
        oc "${kconfig_args[@]}" patch networks.operator.openshift.io cluster --type=merge \
            -p '{"spec":{"defaultNetwork":{"ovnKubernetesConfig":{"gatewayConfig":{"routingViaHost": true}}}}}'
        routing_via_host=$(oc "${kconfig_args[@]}" get networks.operator.openshift.io cluster -o jsonpath='{.spec.defaultNetwork.ovnKubernetesConfig.gatewayConfig.routingViaHost}')
        if [ "${routing_via_host}" != "true" ]; then
            echo "Unable to set local gateway mode for Ambient execution on ${kconfig_display}"
            exit 1
        fi
    done
fi
