...
if [[ "${AMBIENT}" == "true" || "${TEST_SUITE}" == *"ambient"* ]]; then
    ...
    for ctx in "${gw_contexts[@]}"; do
        ctx_args=()
        ctx_display="current context"
        if [[ -n "${ctx}" ]]; then
            ctx_args=("--context=${ctx}")
            ctx_display="${ctx}"
        fi
        echo "Setting local gateway mode on ${ctx_display}"
        oc "${ctx_args[@]}" patch networks.operator.openshift.io cluster --type=merge \
            -p '{"spec":{"defaultNetwork":{"ovnKubernetesConfig":{"gatewayConfig":{"routingViaHost": true}}}}}'
        routing_via_host=$(oc "${ctx_args[@]}" get networks.operator.openshift.io cluster -o jsonpath='{.spec.defaultNetwork.ovnKubernetesConfig.gatewayConfig.routingViaHost}')
