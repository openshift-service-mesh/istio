...
This document describes the high-level architecture of the OpenShift Service Mesh midstream distribution of [Istio](https://istio.io). It tracks upstream Istio with OpenShift-specific customizations, providing traffic management, security (mTLS), and observability for microservices running on Kubernetes.

See also [.github/copilot-instructions.md](.github/copilot-instructions.md) for development workflow, coding conventions, and contribution guidelines.
...
│   │   ├── inject/           #     Sidecar injection (OpenShift-aware)
...
│   ├── profiles/             #   IstioOperator profiles (default, demo, openshift, ambient)
...
| Technology        | Role                                                    |
|-------------------|---------------------------------------------------------|
| Go                | Primary language for all components in this repository  |
| Envoy Proxy       | Data plane proxy (sidecar, gateway, waypoint)           |
| xDS Protocol      | Configuration transport between Istiod and proxies      |
| Kubernetes        | Platform for service discovery, CRDs, and deployment    |
| Helm              | Packaging and deployment of Istio components            |
| Protocol Buffers  | API definitions (istio/api) and internal serialization  |
| SPIFFE            | Workload identity framework for mTLS certificates       |
| Rust              | Ztunnel implementation (separate repository)            |
| Gateway API       | Kubernetes-native API for ingress and mesh gateways     |
...
- **OpenShift** — Platform integration for security context constraints, UID/GID ranges, and platform-specific injection. See `pkg/kube/inject/openshift.go`.
...
### Installation Profiles (`manifests/profiles/`)

- `default.yaml` — Production baseline (Istiod + ingress gateway)
- `demo.yaml` — All features enabled for demonstrations
- `minimal.yaml` — Istiod only
- `ambient.yaml` — Ambient mode (Istiod + CNI + ztunnel)
- `openshift.yaml` — OpenShift-specific settings
- `openshift-ambient.yaml` — Ambient mode on OpenShift

### Platform Profiles (`manifests/helm-profiles/`)

Helm value overrides for specific platforms: `platform-openshift.yaml`, `platform-gke.yaml`, `platform-minikube.yaml`, etc.

### CI/CD

- **Prow** — Primary CI system. Job configs in `prow/config/`. Integration tests run on KinD (`prow/integ-suite-kind.sh`) and OpenShift (`prow/integ-suite-ocp.sh`).
- **GitHub Actions** — PR label enforcement (`.github/workflows/pr-label-check.yml`).
...
Key environment variables: `HUB` (image registry), `TAG` (image tag), `ISTIO` (repo root).
...
- **mTLS everywhere** — All mesh traffic is encrypted with mutual TLS using SPIFFE X.509 certificates issued by Istiod's built-in CA.
- **Certificate rotation** — Certificates are short-lived and automatically rotated by the istio-agent via SDS.
- **Non-root containers** — All Istio containers run as UID 1337 (non-root). Distroless base images minimize attack surface.
- **RBAC** — Each component uses dedicated Kubernetes service accounts with least-privilege roles.
- **CNI privilege** — The CNI plugin requires privileged access to configure network rules; it runs in `kube-system`.
- **Authorization policies** — Istiod enforces `AuthorizationPolicy` CRDs at the proxy level for L4/L7 access control.

---

## OSSM Downstream Customizations

This repository is the midstream fork of [istio/istio](https://github.com/istio/istio) for OpenShift Service Mesh. Key differences from upstream:

- **OpenShift injection** — Sidecar injection respects OpenShift security context constraints (SCC), UID/GID range annotations. See `pkg/kube/inject/openshift.go`.
- **Command overrides** — `istioctl/cmd/ossm_custom_cmd.go` allows disabling specific CLI commands via build-time ldflags for downstream builds.
- **PR labeling** — All PRs require one of: `permanent-change`, `no-permanent-change`, or `pending-upstream-sync` to track upstream synchronization status.
- **Build overrides** — `Makefile.overrides.mk` contains OSSM-specific build customizations.
- **OpenShift profiles** — Dedicated Helm and IstioOperator profiles for OpenShift deployment.
- **Release branches** — Named `release-X.Y.Z-redhat` to distinguish from upstream releases.
...
| **CRD**             | Custom Resource Definition — Kubernetes extension for Istio API types                        |
